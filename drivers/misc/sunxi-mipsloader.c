...
static int mipsloader_probe(struct platform_device *pdev)
{
	...
	/* Write SharedMem address to share registers IMMEDIATELY.
	 * U-Boot wrote these before MIPS boot, but kernel clock framework
	 * may gate BUS_MIPS_CLK during boot, losing register contents.
	 * MIPS reads these once and polls with 0x0 if not set.
	 */
	{
		u32 old_addr = readl(mipsloader_dev->reg_base + SHARE_REG_ADDR_OFF);
		u32 old_size = readl(mipsloader_dev->reg_base + SHARE_REG_SIZE_OFF);
		dev_info(&pdev->dev, "Share regs before: addr=0x%x size=0x%x\n",
			old_addr, old_size);
		if (!old_addr) {
			...
			dev_info(&pdev->dev, "Share regs SET: addr=0x%x size=0x%x\n",
				CPU_COMM_SHMEM_ADDR, CPU_COMM_SHMEM_SIZE);
		} else {
